"""
LCM 2125. Number of Laser Beams in a Bank
...
class Solution:
    def numberOfBeams(self, bank: List[str]) -> int:
        devices = [cnt for b in bank if (cnt := b.count("1")) > 0]

        return sum(devices[i] * devices[i - 1] for i in range(1, len(devices)))


# Time Complexity: O(n) - 8 ms -> 63.46%
# Space Complexity: O(n) - 19.48 MB -> 75.25%
